...
L.Control.TimeLineSlider = L.Control.extend({
  ...
  onAdd: function(map) {
    ...
    this.slider.innerHTML = `<input id="rangeinputslide" type="range" min="1" max="${
      this.options.timelineItems.length
    }" steps="1" value="1"></input>`;
  ...
  getTrackStyle: function(el, sliderLength) {
    ...
    for (var i = 0; i < prefs.length; i++) {
      ...
      } ${coverVal}%, ${that.options.activeColor} ${val}%,  ${
        that.coverBackgroundRGBA
      } 0%, ${that.coverBackgroundRGBA} 100%)}`;
      style +=
        '.range input::-' +
        prefs[i] +
